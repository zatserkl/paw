...
*      use MoTsai,TradMT,URADMT, DEBUG
      ...
      real deltat,deltar,rer,aer,errest,Epl,Eph,Esl,Esh,Kmax,Es1
      real costheta,flag,sum
      integer i,NS,NSminMf,np,nofun

      real     fEstst,fEptst
      external fEstst,fEptst
      logical testEs,testEp,testEsEs,testEsEp
      testEs   = .FALSE.
      testEp   = .FALSE.
      testEsEs = .FALSE.
      testEsEp = .FALSE.
...
*     .. Es1 corresponding to Mfmax
      ...
      if(DEBUG)print*, 'for IdEs: Es,Esl,Esh, Es1:', Es,Esl,Esh, Es1
...
*        .. for test use non-radiative cross section for Es in int. dEs
      ...
      if (Es1.GT.Esh) then
*        .. dEs integration over the whole range
         call quanc8(fEs,Esl,Esh,aer,rer, IdEs,errest,nofun,flag)
         termEs = (del/Ep)**(fp/2.)*IdEs
      else
*        .. split dEs integration
         IdEs = 0.
         if(DEBUG)print*, 'Split integration over Es: Es1,Esh=', Es1,Esh
         if (Es1-Esl .LE. eps) goto 2002
*        .. integral over available Es region
         call quanc8(fEs,Esl,Es1,aer,rer, IdEs,errest,nofun,flag)
 2002    continue

*        .. replace the rest in dEs integration by dEp integration
         IdEsEp = 0.
         IF (testEsEs) THEN
*           .. for test use non-rad. cross section for Es part in int. dEs
            call quanc8(fEstst,Es1,Esh,aer,rer,IdEsEp,errest,nofun,flag)
            goto 2001
         ENDIF
...
*        .. Ep of this process for incident energy Es
         ...
         IF (testEsEp) THEN
*           .. for test use non-rad. cross section for Ep part in int. dEs
            call quanc8(fEptst,Epl,Eph,aer,rer,IdEsEp,errest,nofun,flag)
            goto 2001
         ENDIF
         
         call quanc8(fEp,Epl,Eph,aer,rer,IdEsEp,errest,nofun,flag)
 2001    continue
...
*         endif
  100 enddo
      ...
      sec = 0.
      if(ipoint.LE.1) then
         print*, 'ipoint.LE.1: ipoint =', ipoint
         call WAIT('fEs')
      endif
